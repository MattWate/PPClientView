...
// --- STYLE LOADER ---
// This function adds the necessary stylesheets to the document's <head>
// to ensure the application is styled correctly for the preview.
const loadStyles = () => {
  // Check if styles are already loaded to avoid duplication
  ...
  fontAwesome.id = 'fontawesome-link';
  fontAwesome.rel = 'stylesheet';
  fontAwesome.href = 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css';
  document.head.appendChild(fontAwesome);
};

loadStyles(); // Run the function to load styles

// --- Mocks & Components for Single-File Compilation ---
// To resolve the import errors, the App component and all its dependencies
// are now defined directly within this single, runnable file.
...
const supabase = {
  auth: {
    ...
    signInWithPassword: async ({ email, password }) => {
      ...
      if (password !== 'password') { return { error: { message: 'Invalid credentials. Try password: "password"' } }; }
...
const PublicHomePage = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans">
            ...
            <main>
                <div className="bg-gradient-to-br from-indigo-600 to-purple-600 text-white"><div className="max-w-4xl mx-auto py-20 px-4 text-center"><h2 className="text-4xl font-extrabold sm:text-5xl md:text-6xl">The Future of Cleaning Management</h2><p className="mt-4 max-w-2xl mx-auto text-xl text-indigo-100">Streamline operations, ensure compliance, and deliver pristine results.</p></div></div>
                <div className="py-16 bg-gray-50"><div className="max-w-7xl mx-auto px-4"><div className="text-center mb-12"><h3 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">Simple, Transparent Pricing</h3><p className="mt-4 text-xl text-gray-600">Choose a plan based on your portfolio size. All features included.</p></div><div className="grid gap-8 lg:grid-cols-3 lg:gap-x-8">
                    {pricingTiers.map(tier => (<div key={tier.name} className={`bg-white p-8 rounded-2xl shadow-lg flex flex-col relative ${tier.popular ? 'border-2 border-indigo-600' : ''}`}> {tier.popular && (<div className="absolute top-0 -translate-y-1/2 w-full flex justify-center"><span className="bg-indigo-600 text-white text-xs font-bold uppercase tracking-wider px-4 py-1 rounded-full">Most Popular</span></div>)}<h4 className="text-2xl font-semibold text-gray-900">{tier.name}</h4><p className="mt-2 text-gray-500">{tier.description}</p><p className="mt-6 text-5xl font-bold tracking-tight text-gray-900">{tier.price}<span className="text-xl font-medium text-gray-500"> / month</span></p><ul className="mt-8 space-y-3 flex-grow">{tier.features.map(feature => (<li key={feature} className="flex items-start"><svg className="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg><span className="ml-3 text-gray-600">{feature.startsWith('Up to') ? <span className="font-semibold text-indigo-600">{feature}</span> : feature}</span></li>))}</ul><div className="mt-8"><a href="#" className={`w-full block text-center px-6 py-3 rounded-lg font-semibold ${tier.name === 'Enterprise' ? 'bg-gray-800 text-white hover:bg-gray-900' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>{tier.cta}</a><p className="text-center text-sm text-gray-500 mt-4">One-time setup fee: <span className="font-bold">{tier.onboarding}</span></p></div></div>))}
                </div></div></div>
            </main>
        </div>
    );
