...
import { supabase } from '../services/supabaseClient'; // ✅ use the shared client

// --- Task Management Modal ---
const TaskManagementModal = ({
  area, isOpen, onClose, onUpdate, allCleaners, todaysScheduledTaskCount, profile
}) => {
  ...
  // 🔐 derive site_id from the zone.sites relation we inject below
  ...
  const handleGenerateTasks = async () => {
    ...
    try {
      const newTasks = Array.from({ length: tasksToGenerateCount }).map(() => ({
        ...
        area_id: area.id,
        // If your tasks table DOES NOT have these columns, remove them:
  ...
  const handleCreateAdHocTask = async (e) => {
    ...
    setError('');
    try {
      const { data: { user } } = await supabase.auth.getUser(); // ✅ same client, so session is shared
          ...
          <div className="mb-8 p-4 bg-gray-50 rounded-lg">
            ...
            {tasksToGenerateCount > 0 ? (
              <div className="space-y-3">
                <p>
                  This area requires <span className="font-bold">{tasksToGenerateCount}</span> more scheduled cleaning task(s) today.
                </p>
                <button
          ...
          <div className="mb-8">
            ...
            {pendingTasks.length > 0 ? (
              <div className="space-y-4">
                <p>
                  Assign all <span className="font-bold">{pendingTasks.length}</span> pending task(s) to a cleaner:
                </p>
                <select
                  value={assigneeId}
...
export default function SupervisorDashboard({ profile }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: tasksData, error: tasksError } = await supabase
        .from('tasks')
        .select('id, title, status, area_id, assigned_to, task_type, created_at, zone_id') // zone_id optional
        .in('zone_id', zoneIds)
  ...
  return (
    <>
      ...
      <TaskManagementModal
        profile={profile}
        // inject zone object (with `sites`) so the modal can read site id/name
